...
def test_deduped_state_writes_are_never_called_on_the_event_loop():
    ...
    All three doc-state tables now record that state under ``BEGIN IMMEDIATE``,
    and acquiring the write lock is a blocking wait on any concurrent writer --
    up to the connection's busy timeout. On the loop thread that is the same stall
    this whole change exists to remove, so a new call site added straight to a
    coroutine body must fail here rather than in production.

    One assertion for three modules: ``folder_watcher``, ``artifact_ingest`` and
    ``agent_source`` each name their helper ``_record_deduped_state``.
    """
    offenders = _on_loop_call_sites("_record_deduped_state")
    assert offenders == [], (
        "_record_deduped_state takes BEGIN IMMEDIATE on the event loop at:\n  "
        + "\n  ".join(offenders)
        + "\nOffload it: await run_to_completion(lambda: _record_deduped_state(...)). "
          "run_to_completion rather than a bare to_thread, so a cancellation while "
          "the work item is still queued cannot drop a terminal state write and "
          "leave the document mid-ingest."
    )
