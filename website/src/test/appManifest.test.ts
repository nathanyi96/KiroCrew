...
describe('APP_MANIFEST_KEY', () => {
  ...
  it('carries full literal keys, which is the only form check-i18n-keys resolves', () => {
    for (const [name, keys] of Object.entries(APP_MANIFEST_KEY)) {
      // `pageLabel` is optional: an app with no dashboard page has no label key.
      const present = [keys.displayName, keys.description, ...keys.highlights]
      if (keys.pageLabel) present.push(keys.pageLabel)
