...
async def api_chat_slot_slack_pause(request: web.Request) -> web.Response:
    ...
    # Off the loop: every SessionMap mutation rewrites the whole map, so a large
    # map turns this endpoint into synchronous JSON + file I/O on the gateway
    # loop and stalls chat and heartbeat behind it. Offloading a single write was
    # tried and reverted once (#2976) because back then the event loop was the
    # only thing serializing read-modify-write. It is safe now: main's
    # ``_MAP_LOCK`` (#2989) supplies that serialization, every mutator including
    # this one holds it, and the ratchet's only await restriction is inside a
    # ``batched_save`` block — which this is not.
    was_paused = bool(
        await asyncio.to_thread(state.sessions.set_slack_paused, session_key, paused)
    )
