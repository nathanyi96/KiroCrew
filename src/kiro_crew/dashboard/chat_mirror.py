...
async def api_chat_slot_mirror_pause(request: web.Request) -> web.Response:
    ...
    # Off the loop for the same reason as the Slack twin — see the note there for
    # why #2976's revert does not apply once ``_MAP_LOCK`` exists.
    was_paused = bool(
        await asyncio.to_thread(state.sessions.set_mirror_paused, session_key, paused)
    )
