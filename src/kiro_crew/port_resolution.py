...
Historically the chain lived in ``cli_server`` and a large body of tests
patches these symbols there (``kiro_crew.cli_server._gateway_owns_port`` and
friends). Chain-internal calls therefore resolve their callee through
:func:`_patchable`, which prefers the ``cli_server`` namespace whenever that
module is loaded and falls back to this module otherwise. In production the
two namespaces hold the very same function objects (``cli_server`` re-exports
them), so the lookup is behaviour-neutral; under test it is what lets a patch
in the ``cli_server`` namespace intercept a call made from inside the chain.
"""
